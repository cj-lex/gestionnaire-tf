...
# ---------------------------------------------------------------------------
# Configuration e-mail (variables d'environnement)
...
def audit(action: str, details: str):
    """Écrit une ligne d'audit horodatée dans audit.log (réseau)."""
    try:
        DATA_DIR.mkdir(parents=True, exist_ok=True)
        ts = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        ligne = f"[{ts}] {action} | {details}\n"
        with _lock:
            with open(AUDIT_FILE, "a", encoding="utf-8") as fh:
                fh.write(ligne)
    except Exception as exc:
        print(f"[AUDIT] Impossible d'écrire dans {AUDIT_FILE}: {exc}", file=sys.stderr)
...
def utiliser():
    ...
    audit("ATTRIBUTION", f"timbre={timbre['numero']} dossier={dossier} clerc={code_clerc}")
...
def admin_modifier_dossier():
    ...
    if not timbre:
        ...
        if year_old != year_new:
            ...
            save_timbre(timbre)
        notifier_attribution(timbre)
        audit("ADMIN_MODIF", f"timbre={timbre['numero']} dossier={dossier} clerc={code_clerc} date_achat={date_achat_new} date_util={date_util}")
